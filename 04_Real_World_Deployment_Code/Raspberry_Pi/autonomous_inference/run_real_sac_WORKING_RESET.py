...
# Manual rescue boost only, not automatic every few seconds
MANUAL_BOOST_THROTTLE = 0.80
MANUAL_BOOST_SECONDS = 0.35

# Small start boost to overcome static friction
...
def obs_to_video_frame(obs, armed, raw_steering, sent_steering, throttle):
    # obs shape: (6, 80, 160), normalized RGB float32
...
def main():
    ...
    print("\nREAL SAC WORKING RESET")
    print("Clean version: no offset, no anti-wall, no smoothing memory.")
    print("")
    print("Controls:")
        ...
        with NonBlockingKeyboard() as keyboard:
            while True:
                ...
                if key == "g":
                    ...
                    print(f"\nfixed_throttle increased to {fixed_throttle:.3f}\n")
                    ...
                    print(f"\nfixed_throttle decreased to {fixed_throttle:.3f}\n")

                elif key == "b":
                    manual_boost_until = time.time() + MANUAL_BOOST_SECONDS
                    print(
                        f"\nMANUAL BOOST for {MANUAL_BOOST_SECONDS:.2f}s "
                        f"at throttle {MANUAL_BOOST_THROTTLE:.2f}\n"
                    )
